...
private[spark] class ShmShuffleFetcherKeyValuesIterator
  ...
  private val kvBuffer =
               new Array[(Object, Seq[Object])](SHUFFLE_MERGED_KEYVALUE_PAIRS_RETRIEVAL_SIZE)
  private var count = 0
  private var actualPairs = 0
   ...
   private  val vvalues = new ArrayList[ArrayList[Object]]()
   ...
   private def fetch(): Unit = {
     ...
     kvalueTypeId match {
       case  ShuffleDataModel.KValueTypeId.Int => {
         ...
         actualPairs= reduceShuffleStore.getKVPairsWithIntKeys(
       ...
       case  ShuffleDataModel.KValueTypeId.Float => {
         ...
         actualPairs= reduceShuffleStore.getKVPairsWithFloatKeys(
                         fkvalues, vvalues, SHUFFLE_MERGED_KEYVALUE_PAIRS_RETRIEVAL_SIZE)
         //now, push the data to the kvbuffer.
         for (i <- 0 to actualPairs-1) {
           val vv = vvalues(i).seq
           kvBuffer (i) = (fkvalues(i), vv)
         }
       ...
       case  ShuffleDataModel.KValueTypeId.Long => {
         ...
         actualPairs= reduceShuffleStore.getKVPairsWithLongKeys(
                         lkvalues, vvalues,SHUFFLE_MERGED_KEYVALUE_PAIRS_RETRIEVAL_SIZE)
         //now, push the data to the kvbuffer.
         for (i <- 0 to actualPairs-1) {
           val vv = vvalues(i).seq
           kvBuffer(i) = (lkvalues(i), vv)
         }
